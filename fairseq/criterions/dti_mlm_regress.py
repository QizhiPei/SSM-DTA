...
@register_criterion("dti_mlm_regress")
class DTIMaskedLmRegressLoss(FairseqCriterion):
    """
    ...
    def forward(self, model, sample, args, reduce=True):
        ...
        masked_tokens_0 = sample["target"]["tgt_tokens_0"].ne(self.padding_idx)
        sample_size_mlm_0 = masked_tokens_0.int().sum()

        masked_tokens_1 = sample["target"]["tgt_tokens_1"].ne(self.padding_idx)
        ...
        logits_mlm_0 = model.encoder_0(sample["net_input"]["src_tokens_0"], masked_tokens=masked_tokens_0)[0]

        logits_mlm_1 = model.encoder_1(sample["net_input"]["src_tokens_1"], masked_tokens=masked_tokens_1)[0]

        # targets = model.get_targets(sample, [logits])
        targets_mlm_0 = sample["target"]["tgt_tokens_0"]
        targets_mlm_1 = sample["target"]["tgt_tokens_1"]
