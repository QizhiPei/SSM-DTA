...
@register_model("roberta_dti_separate_encoder")
# class RobertaDTI(RobertaModel):
class RobertaDTI(BaseFairseqModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        if classification_head_name is not None:
            tmp = torch.cat((x_0[:, 0, :], x_1[:, 0, :]), 1).unsqueeze(1)
            x = self.classification_heads[classification_head_name](tmp)
        # extra_0 is not used
        return x, extra_0
...
@register_model_architecture(
    "roberta_dti_separate_encoder", "roberta_dti_separate_encoder"
)
